...
async function generateMeshy3DModel(prompt) {
  if (!MESHY_API_KEY) {
    console.log('[Meshy AI] MESHY_API_KEY não configurada no ambiente. Usando pipeline de fallback HD...');
    return null;
  }

  console.log(`[Meshy AI] Enviando requisição para gerar modelo 3D: "${prompt}"...`);
  ...
  return new Promise((resolve) => {
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log('[Meshy AI] Resposta da API:', json);
          resolve(json.result || json);
        } catch (e) {
          console.error('[Meshy AI] Erro ao parsear resposta:', e);
          resolve(null);
        }
      });
    });
    ...
    req.on('error', (e) => {
      console.error('[Meshy AI] Erro na requisição HTTPS:', e.message);
      resolve(null);
    });
