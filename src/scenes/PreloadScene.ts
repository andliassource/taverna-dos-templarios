...
export class PreloadScene extends Phaser.Scene {
  ...
  private generateHDSpritesheet(
    ...
    // Se a textura real for encontrada, copia. Senão, utiliza o Paladino HD como padrão.
    const fallbackKey = this.textures.exists('PALADIN-sheet') ? 'PALADIN-sheet' : '';
    if (fallbackKey) {
      const realTex = this.textures.get(fallbackKey);
      const srcImg = realTex.getSourceImage() as HTMLImageElement;
      if (srcImg) {
        this.textures.addImage(key, srcImg);
      }
    }
  }
