import Phaser from 'phaser';
import { UI_THEME } from '../../config/theme.config';
import { AchievementSystem } from '../../systems/AchievementSystem';
import { SoundSynth } from '../../utils/SoundSynth';

export class AchievementModal extends Phaser.GameObjects.Container {
  constructor(scene: Phaser.Scene, onClose: () => void, onRefresh: () => void) {
    const { width, height } = scene.scale;
    super(scene, width / 2, height / 2);
    this.setDepth(210);

    const system = AchievementSystem.getInstance();
    const achievements = system.getAchievements();

    const pw = 520;
    const ph = 400;
    const px = -pw / 2;
    const py = -ph / 2;

    const bg = scene.add.graphics();
    bg.fillStyle(UI_THEME.colors.bgDark, 0.96);
    bg.fillRoundedRect(px, py, pw, ph, 10);
    bg.lineStyle(2, UI_THEME.colors.borderGold, 0.95);
    bg.strokeRoundedRect(px, py, pw, ph, 10);
    bg.lineStyle(1, UI_THEME.colors.borderDarkGold, 0.7);
    bg.strokeRoundedRect(px + 3, py + 3, pw - 6, ph - 6, 8);
    this.add(bg);

    const title = scene.add.text(0, py + 22, `🏆 CONQUISTAS & TÍTULOS MEDIEVAIS`, {
      fontFamily: UI_THEME.fonts.title, fontSize: '16px', fontStyle: 'bold', color: UI_THEME.colors.textGold,
      stroke: '#000000', strokeThickness: 3,
    }).setOrigin(0.5);
    this.add(title);

    const subtitle = scene.add.text(0, py + 44, `TÍTULO ATUAL: [ ${system.getActiveTitle()} ]`, {
      fontFamily: UI_THEME.fonts.title, fontSize: '11px', fontStyle: 'bold', color: UI_THEME.colors.textBlue,
    }).setOrigin(0.5);
    this.add(subtitle);

    const closeBtn = scene.add.text(px + pw - 26, py + 14, '✖', {
      fontFamily: UI_THEME.fonts.title, fontSize: '16px', color: UI_THEME.colors.textRed
    }).setOrigin(0.5).setInteractive({ useHandCursor: true });
    closeBtn.on('pointerdown', onClose);
    this.add(closeBtn);

    // Lista de Conquistas
    const listY = py + 68;
    const itemH = 58;

    achievements.forEach((ach, index) => {
      const sy = listY + index * (itemH + 6);

      const itemBg = scene.add.graphics();
      itemBg.fillStyle(ach.unlocked ? 0x18102a : 0x0c0714, 0.9);
      itemBg.fillRoundedRect(px + 20, sy, pw - 40, itemH, 6);
      itemBg.lineStyle(1, ach.unlocked ? UI_THEME.colors.borderGold : 0x332244, 0.7);
      itemBg.strokeRoundedRect(px + 20, sy, pw - 40, itemH, 6);
      this.add(itemBg);

      const icon = scene.add.text(px + 45, sy + itemH / 2, ach.icon, { fontSize: '20px' }).setOrigin(0.5);
      const achTitle = scene.add.text(px + 70, sy + 10, ach.title, {
        fontFamily: UI_THEME.fonts.title, fontSize: '12px', fontStyle: 'bold',
        color: ach.unlocked ? UI_THEME.colors.textGold : '#888888',
      });
      const achDesc = scene.add.text(px + 70, sy + 28, `${ach.description} (${ach.progress}/${ach.maxProgress})`, {
        fontFamily: UI_THEME.fonts.body, fontSize: '10px', color: '#aaaaaa',
      });

      this.add([icon, achTitle, achDesc]);

      if (ach.unlocked && !ach.claimed) {
        const claimBtn = scene.add.text(px + pw - 90, sy + itemH / 2, 'RESGATAR', {
          fontFamily: UI_THEME.fonts.title, fontSize: '10px', fontStyle: 'bold', color: '#ffffff',
          backgroundColor: '#008844', padding: { x: 10, y: 6 }
        }).setOrigin(0.5).setInteractive({ useHandCursor: true });
        claimBtn.on('pointerdown', () => {
          system.claimReward(ach.id);
          SoundSynth.playClick();
          onRefresh();
        });
        this.add(claimBtn);
      } else if (ach.claimed) {
        const statusTxt = scene.add.text(px + pw - 90, sy + itemH / 2, 'CONCLUÍDO ✅', {
          fontFamily: UI_THEME.fonts.body, fontSize: '10px', color: '#00ffcc'
        }).setOrigin(0.5);
        this.add(statusTxt);
      } else {
        const lockTxt = scene.add.text(px + pw - 90, sy + itemH / 2, 'BLOQUEADO 🔒', {
          fontFamily: UI_THEME.fonts.body, fontSize: '10px', color: '#666666'
        }).setOrigin(0.5);
        this.add(lockTxt);
      }
    });

    scene.add.existing(this);
  }
}
