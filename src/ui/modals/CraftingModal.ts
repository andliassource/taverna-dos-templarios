...
export class CraftingModal extends Phaser.GameObjects.Container {
  constructor(scene: Phaser.Scene, onClose: () => void, onRefresh: () => void) {
    const { width, height } = scene.scale;
    super(scene, width / 2, height / 2);
    this.setDepth(210);

    const cs = CraftingSystem.getInstance();
    ...
    const title = scene.add.text(0, py + 22, `🔨 FORJA E FORJAMENTO TEMPLÁRIO`, {
    ...
    recipes.forEach((rec, index) => {
      ...
      const reqText = rec.materials.map(m => `${m.item}: ${m.amount}`).join(', ');
      const rMaterials = scene.add.text(px + 75, sy + 28, `Materiais: ${reqText}`, {
        fontFamily: UI_THEME.fonts.body, fontSize: '10px', color: '#aaaaaa',
      });
      const rCost = scene.add.text(px + 75, sy + 44, `Bônus: ${rec.resultItem.statBonus} (${rec.resultItem.rarity})`, {
        fontFamily: UI_THEME.fonts.hud, fontSize: '9px', color: UI_THEME.colors.textBlue,
      });

      this.add([icon, rName, rMaterials, rCost]);

      const craftBtn = scene.add.text(px + pw - 90, sy + itemH / 2, 'FORJAR 🔨', {
        fontFamily: UI_THEME.fonts.title, fontSize: '10px', fontStyle: 'bold', color: '#ffffff',
        backgroundColor: '#663300', padding: { x: 10, y: 6 }
      }).setOrigin(0.5).setInteractive({ useHandCursor: true });
