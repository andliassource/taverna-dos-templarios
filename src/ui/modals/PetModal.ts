import Phaser from 'phaser';
import { UI_THEME } from '../../config/theme.config';
import { PetSystem, PetItem } from '../../systems/PetSystem';
import { SoundSynth } from '../../utils/SoundSynth';

export class PetModal extends Phaser.GameObjects.Container {
  constructor(scene: Phaser.Scene, onClose: () => void, onRefresh: () => void) {
    const { width, height } = scene.scale;
    super(scene, width / 2, height / 2);
    this.setDepth(210);

    const ps = PetSystem.getInstance();
    const items = ps.getItems();
    const activePet = ps.getActivePet();
    const activeMount = ps.getActiveMount();

    const pw = 520;
    const ph = 390;
    const px = -pw / 2;
    const py = -ph / 2;

    this.setScale(0.85);
    this.setAlpha(0);
    scene.tweens.add({
      targets: this, scaleX: 1, scaleY: 1, alpha: 1, duration: 200, ease: 'Back.out',
    });

    const bg = scene.add.graphics();
    bg.fillStyle(UI_THEME.colors.bgDark, 0.96);
    bg.fillRoundedRect(px, py, pw, ph, 10);
    bg.lineStyle(2, UI_THEME.colors.borderGold, 0.95);
    bg.strokeRoundedRect(px, py, pw, ph, 10);
    bg.lineStyle(1, UI_THEME.colors.borderDarkGold, 0.7);
    bg.strokeRoundedRect(px + 3, py + 3, pw - 6, ph - 6, 8);
    this.add(bg);

    const title = scene.add.text(0, py + 22, `🐾 MASCOTES E MONTARIAS DA ORDEM`, {
      fontFamily: UI_THEME.fonts.title, fontSize: '16px', fontStyle: 'bold', color: UI_THEME.colors.textGold,
      stroke: '#000000', strokeThickness: 3,
    }).setOrigin(0.5);
    this.add(title);

    const closeBtn = scene.add.text(px + pw - 26, py + 14, '✖', {
      fontFamily: UI_THEME.fonts.title, fontSize: '16px', color: UI_THEME.colors.textRed
    }).setOrigin(0.5).setInteractive({ useHandCursor: true });
    closeBtn.on('pointerdown', onClose);
    this.add(closeBtn);

    const listY = py + 58;
    const itemH = 62;

    items.forEach((item: PetItem, index: number) => {
      const sy = listY + index * (itemH + 6);

      const itemBg = scene.add.graphics();
      itemBg.fillStyle(0x140d24, 0.9);
      itemBg.fillRoundedRect(px + 20, sy, pw - 40, itemH, 6);
      itemBg.lineStyle(1, UI_THEME.colors.borderGold, 0.7);
      itemBg.strokeRoundedRect(px + 20, sy, pw - 40, itemH, 6);
      this.add(itemBg);

      const icon = scene.add.text(px + 45, sy + itemH / 2, item.icon, { fontSize: '24px' }).setOrigin(0.5);
      const pName = scene.add.text(px + 75, sy + 10, `${item.name} [${item.tier}]`, {
        fontFamily: UI_THEME.fonts.title, fontSize: '12px', fontStyle: 'bold', color: UI_THEME.colors.textGold,
      });
      const pDesc = scene.add.text(px + 75, sy + 28, item.description, {
        fontFamily: UI_THEME.fonts.body, fontSize: '10px', color: '#aaaaaa',
      });
      const pBuff = scene.add.text(px + 75, sy + 44, `Bônus Ativo: ${item.buffText}`, {
        fontFamily: UI_THEME.fonts.hud, fontSize: '9px', color: UI_THEME.colors.textBlue,
      });

      this.add([icon, pName, pDesc, pBuff]);

      const isEquipped = (activePet?.id === item.id) || (activeMount?.id === item.id);
      const equipBtn = scene.add.text(px + pw - 90, sy + itemH / 2, isEquipped ? 'EQUIPADO ✅' : 'EQUIPAR 🐾', {
        fontFamily: UI_THEME.fonts.title, fontSize: '10px', fontStyle: 'bold', color: '#ffffff',
        backgroundColor: isEquipped ? '#008844' : '#552288', padding: { x: 10, y: 6 }
      }).setOrigin(0.5).setInteractive({ useHandCursor: true });
      equipBtn.on('pointerdown', () => {
        ps.equip(item.id);
        SoundSynth.playClick();
        onRefresh();
      });
      this.add(equipBtn);
    });

    scene.add.existing(this);
  }
}
